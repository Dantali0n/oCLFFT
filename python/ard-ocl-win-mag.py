...
plt.bar(index, ywin, bar_width,
        alpha=opacity, color='#cc0000', label='window-ocl')
plt.bar(index, ymag, bar_width,
        alpha=opacity, color='#0000cc', label='magnitude-ocl', bottom=ywin)
...
plt.bar(index + (bar_width * 1.1), zmag, bar_width,
        alpha=opacity, color='#000088', label='magnitude-fftw', bottom=zwin)

plt.xlabel('Input size')
plt.ylabel('Mean execution time in seconds')
plt.title('Performance of basic OpenCL compared to sequential CPU')
plt.xticks(index + ((0.9 / num_bars) * (num_bars / 5)), ("8", "128", "512",  "2048", "8192", "32768", "131072", "524288", "2097152"))
plt.legend()

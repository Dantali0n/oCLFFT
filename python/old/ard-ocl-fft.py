...
for f in x[0]:
    ...
    zfft.append(sum(data['fft'].values) / len(data['window']) / 1000000)
...
plt.bar(index, yfft, bar_width,
        alpha=opacity, color='#cc0000', label='fft-ocl')

plt.bar(index + (bar_width * 1.1), zfft, bar_width,
        alpha=opacity, color='#880000', label='fft-fftw')

plt.xlabel('Input size')
plt.ylabel('Mean execution time in seconds')
plt.title('Performance of lookup OpenCL (without bit-reverse!) compared to fftw')
plt.xticks(index + ((0.9 / num_bars) * (num_bars / 5)), ("131072", "524288", "2097152"))
plt.legend()
